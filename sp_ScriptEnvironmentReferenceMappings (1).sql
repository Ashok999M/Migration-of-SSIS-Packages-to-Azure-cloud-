...
CREATE PROCEDURE [dbo].[usp_SSIS_ScriptEnvironmentReferenceMappings]
          @folder sysname
AS
        SET NOCOUNT ON;
		...
		/* Generate the Environment references creation */        SELECT [cmd] = @tab + 'IF NOT EXISTS (SELECT 1 FROM [SSISDB].[internal].[object_parameters] WHERE parameter_name = ''' + op.parameter_name + ''' and project_id = ''' + convert(varchar,prj.project_id) + ''' and object_name = ''' + op.object_name + ''')' + @cr
										...
										+ @tab + @tab + ', @value_type = '+ op.value_type + @cr
        ...
        FROM [internal].[object_parameters] op 
			  INNER JOIN [internal].[projects] prj ON op.project_id=prj.project_id
			  INNER JOIN [internal].[folders] fol ON fol.folder_id=prj.folder_id
